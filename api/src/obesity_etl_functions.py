...
from src.data_common_functions import read_file, select_data
from src.bd import CURRENT_DIR

# Cálculo age_range

def calc_age_range(age):
...
#   transform: Boolean que indica si se va o no ejecutar la función transform_calcul
...
def load_clean_transform(columns_to_drop, columns_to_leave, transform=True):
    ...
    # Si la cantidad de registros con valores nulos es menos de 1% los elimino
    # Es el caso del dataset utilizado y para el subconjunto de datos seleccionado
    ...
    if transform:
        # Aplico el pipeline que transforma los datos y calcula las variables
